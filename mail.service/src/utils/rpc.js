...
const { sendMail } = require("../sendmail/send.mail")
...
const sendEmailFunction = async (requestedData) => {
    try {
        console.log("=====>", requestedData);
        const { email, subject, body } = requestedData
...
const functionResponse = async (requestedData) => {
    ...
    switch (event) {
        case 'SEND_EMAIL':
            const responseBack = await sendEmailFunction(data)
            return responseBack
    }
}
